...
| 屬性投票送出 | 先以 response_id 去重，再取得短期全域寫入鎖，最後以單一 D1 batch 原子寫入 | 最多讀 response 去重 2 次、上下文 1 列、兩個 state；最多寫 3 個事件、2 個 state、1 個配對統計、1 個 feed，加上鎖的取得／釋放；總量仍遠低於 100 列 |
