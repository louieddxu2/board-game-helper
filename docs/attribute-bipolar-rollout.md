...
目前完成結構相容、方向轉換、投票 UI、現有瀏覽介面、唯讀換算預覽，以及第六步一次性正式轉換的本地實作。遠端尚未執行合併 migration。
...
正式啟用已改採一次性資料轉換：migration 0088 直接把兩個來源屬性的有效狀態換算成新雙極屬性，合併候選與公開 snapshot，停用來源屬性，並讓封存 raw response/event 不再參與執行流程。遠端尚未執行 migration。

已整理[啟用設計與必要驗收](attribute-win-activation-plan.md)：目前驗收以一次性轉換後的新屬性、候選欄位、目錄 snapshot、出題與重建流程為準，不再要求舊資料映射成新票。

第六步已完成本地實作：migration 0088 會一次建立「取勝方式」雙極屬性、換算現有分數、合併候選欄位、停用兩個來源屬性並重建公開目錄。舊 response/event 原始列保留作封存，但不再參與出題、活動或遊戲合併重建；不填入舊題 mapping。0085～0087 的 baseline／mapping 結構保留為未來可清理的相容基礎，正式環境仍尚未執行 0088。
