...
目前完成結構相容、方向轉換、投票 UI、現有瀏覽介面、唯讀換算預覽，以及第六步一次性正式轉換；0092 清理與後續屬性目錄同步修正也已發布正式環境。
...
- 題目沿用原本「哪款遊戲的『屬性』＋既有問句結尾」格式，端點只替換屬性名稱；說明使用本題高分端的 `shortDescription ?? fullDescription`，不改寫既有原文。
- 範例與直接評分軸皆標示本題 0／10 對應的概念；反向題的範例使用 `10 - score`，不改動快取。
- 遊戲卡片維持原本可見內容，不在按鈕內增加端點文字；題目、評分軸與投票回饋辨識本題方向，鍵盤與拖曳評分同樣以本題方向操作。
...
正式啟用已改採一次性資料轉換：migration 0088 直接把兩個來源屬性的有效狀態換算成新雙極屬性，合併候選與公開 snapshot，停用來源屬性，並讓封存 raw response/event 不再參與執行流程。正式環境已完成 0088–0092。

已整理[啟用設計與必要驗收](attribute-win-activation-plan.md)：目前驗收以一次性轉換後的新屬性、候選欄位、目錄 snapshot、出題與重建流程為準，不再要求舊資料映射成新票。

第六步已完成正式實作：migration 0088 一次建立「取勝方式」雙極屬性、換算現有分數、合併候選欄位、停用兩個來源屬性並重建公開目錄。舊 response/event 原始列轉為新屬性格式後保留作完整投票歷史，但不再參與舊屬性流程；0092 已清理未使用的 baseline／mapping 相容表。

部署需先套用 migration 0083、0084、0085、0086、0087、0088，再啟用讀取新雙極屬性的程式；本步不執行正式環境 migration 或部署。
