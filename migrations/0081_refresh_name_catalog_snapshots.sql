-- 0080 updates the canonical game rows and emits bounded catalog deltas.
-- Rebuild both compact snapshots once so a fresh client sees the corrected
-- names immediately, without waiting for its first delta synchronization.
...
SELECT 81, chunk_number, games_json FROM grouped;

INSERT INTO game_catalog_snapshot_chunks (generation, chunk_number, games_json)
SELECT 81, 0, '[]'
WHERE NOT EXISTS (SELECT 1 FROM game_catalog_snapshot_chunks WHERE generation = 81);
...
SELECT 81, chunk_number, entries_json
FROM grouped;

INSERT INTO attribute_catalog_snapshot_chunks (generation, chunk_number, entries_json)
SELECT 81, 0, '[]'
WHERE NOT EXISTS (
  SELECT 1 FROM attribute_catalog_snapshot_chunks WHERE generation = 81
);
