...
export const RuleCard = ({ rule, gameName, gameHref, onEdit, onTagClick, gameId }: { rule: RuleCardType; gameName?: string; gameHref?: string; onEdit?: () => void; onTagClick?: (tag: string) => void; gameId?: string }) => {
  ...
  const displayStage = rule.flowStage && rule.flowStage !== 'uncategorized' ? stageNames[rule.flowStage] : undefined;
  ...
  return <article className="rule-card" id={`rule-${rule.id}`}>
    <div className="rule-meta">
      ...
      {displayStage && <span>{displayStage}</span>}
    </div>
    ...
    {rule.tags.length > 0 && <div className="rule-tags" aria-label="主題標籤">{rule.tags.slice(0, 4).map((tag) => onTagClick
    ? <button type="button" className="tag-chip" key={tag.id} onClick={() => onTagClick(tag.name)}>#{tag.name}</button>
    : <span className="tag-chip" key={tag.id}>#{tag.name}</span>)}{rule.tags.length > 4 && <span className="tag-overflow">＋{rule.tags.length - 4}</span>}</div>}
  <div className="rule-notes">
    ...
    <button type="button" className="text-action copy-link-btn" onClick={copyRuleLink} title="複製這條規則的分享連結">🔗 複製連結</button>
  </div>
