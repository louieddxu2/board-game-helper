    ...
    >
      <table className="attributes-matrix">
        ...
        <tbody>{sortedRows.map((row) => {
          ...
          return <tr key={row.id} className={`${row.kind === 'pending' ? 'attributes-matrix-pending ' : ''}${isAnchor ? 'attributes-matrix-anchor' : ''}`.trim() || undefined}><th scope="row" className="attributes-matrix-subject">{row.kind === 'processed' ? <button type="button" className="attributes-matrix-subject-button" aria-pressed={isAnchor} onClick={() => selectSimilarityAnchor(row)}>{row.displayName}</button> : <span>{row.displayName}</span>}<small>{statusLabel}</small></th>{row.values.map((value, index) => { const bipolar = attributeDisplayEndpoints(payload.attributes[index]); return <td key={payload.attributes[index]?.id ?? index} className={`attributes-matrix-value ${scoreClass(value)}`} title={row.details[index] ?? '尚無資料'} aria-label={value != null && bipolar ? `${formatScore(value)}；0＝${bipolar.low.label}，10＝${bipolar.high.label}` : undefined}>{formatScore(value)}</td>; })}</tr>;
        })}</tbody>
      </table>
    </div>
  </section>;
