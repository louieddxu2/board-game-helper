...
const valueDetails = (value: AttributesPayload['values'][number] | undefined) => {
  ...
  const pairwise = value.comparisonScore == null ? '' : `；比較推算 ${value.comparisonScore.toFixed(1)}`;
  return `最終 ${value.score.toFixed(1)}；${direct}；${comparison}${pairwise}`;
};
...
export const AttributeMatrixTable = ({ payload }: { payload: AttributesPayload }) => {
  ...
  return <section className="attributes-table-card" aria-labelledby="attributes-table-heading">
    ...
    <p className="attributes-table-description">分數是直接評分與兩兩比較的合成結果；將游標移到格子上可查看兩種資料各自的貢獻。尚未處理的來源項目會保留在表內，但不會冒充已對應遊戲。</p>
