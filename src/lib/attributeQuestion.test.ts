...
describe('attributeQuestionEnding', () => {
  test('provides intentional wording for every current attribute', () => {
    expect(Object.keys(ATTRIBUTE_QUESTION_ENDINGS)).toHaveLength(27);
  ...
  test('keeps the merged win-method question in the original score-weight wording', () => {
    expect(attributeQuestionEnding('win_method')).toBe('比重較高');
  });
