...
export const calculateAxisMarkerPosition = (stageWidth: number, markerWidth: number, score: number): AxisMarkerPosition => {
  const safeStageWidth = Math.max(0, stageWidth);
  const safeMarkerWidth = Math.min(Math.max(0, markerWidth), safeStageWidth);
  const target = safeStageWidth * Math.max(0, Math.min(10, score)) / 10;
