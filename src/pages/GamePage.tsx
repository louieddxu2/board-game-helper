...
export const GamePage = () => {
  ...
  return <section className="game-page">
    <header className="game-hero">
      ...
      {canEdit && <div className="inline-actions"><button type="button" className="button secondary" onClick={() => setEditingGame(true)}>編輯遊戲名稱</button><Link className="button primary" to="/add">＋新增規則</Link></div>}
    </header>
    {justAdded && <div className="success-banner">已經記下 {justAdded} 條規則。下次開桌前，它們會在這裡等你。</div>}
    <nav className="stage-tabs" aria-label="規則流程分類">
      <button type="button" className={activeStage === 'all' ? 'active' : ''} onClick={() => setActiveStage('all')}>全部</button>
      {stages.map((stage) => <button type="button" key={stage} className={activeStage === stage ? 'active' : ''} onClick={() => setActiveStage(stage)}>{stageNames[stage]}</button>)}
    </nav>
    <div className="game-rules">
      {visibleRules.map((rule) => <RuleCard key={rule.id} rule={rule} onEdit={canEdit ? () => setEditing(rule) : undefined} />)}
      {visibleRules.length === 0 && <p className="empty-state">這個分類還沒有規則。</p>}
    </div>
...
const RuleEditor = ({ rule, onClose, onSaved }: { rule: RuleCardType; onClose(): void; onSaved(): Promise<void> }) => {
  ...
  const save = async () => {
    setSaving(true);
    try {
      await api.patchRule(rule.id, { statement, commonMistake: commonMistake || null, details: details || null, flowStage, playerCountNote: playerCountNote || null, editionNote: editionNote || null, isFeatured });
      await onSaved();
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="modal" role="dialog" aria-modal="true" aria-labelledby="edit-rule-title">
      <div className="modal-heading"><h2 id="edit-rule-title">編輯規則</h2><button type="button" onClick={onClose}>×</button></div>
...
const GameEditor = ({ game, onClose, onSaved }: { game: GameDetail; onClose(): void; onSaved(): Promise<void> }) => {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="modal" role="dialog" aria-modal="true" aria-labelledby="edit-game-title">
      <div className="modal-heading"><h2 id="edit-game-title">編輯遊戲名稱</h2><button type="button" onClick={onClose}>×</button></div>
