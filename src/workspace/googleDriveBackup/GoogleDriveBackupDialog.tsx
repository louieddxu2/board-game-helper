...
import type { WorkspaceDriveBackupRecord } from './useWorkspaceGoogleDriveBackup';

type BackupStatus = 'offline' | 'never' | 'dirty' | 'saved';
...
const statusLabel = (status: BackupStatus) => status === 'offline' ? '目前離線' : status === 'dirty' ? '有未備份變更' : status === 'saved' ? '已完成備份' : '尚未備份';
...
export const GoogleDriveBackupDialog = ({ status, busy, message, error, record, remoteFile, authorized, onClose, onConnect, onBackup, onFindRemote, onRestore, onDisconnect }: GoogleDriveBackupDialogProps) => {
  ...
  return <WorkspaceModal title="Google Drive" onClose={onClose} className="workspace-google-drive-dialog">
    ...
    <div className="workspace-google-drive-actions">
      ...
      <ActionButton onClick={onBackup} disabled={isBusy || status === 'offline'}><WorkspaceIcon name="upload" size={18} />立即備份</ActionButton>
      <ActionButton onClick={onFindRemote} disabled={isBusy || status === 'offline'}><WorkspaceIcon name="download" size={18} />尋找雲端備份</ActionButton>
      {remoteFile && <ActionButton onClick={onRestore} disabled={isBusy || status === 'offline'} variant="primary"><WorkspaceIcon name="check" size={18} />使用此備份</ActionButton>}
    </div>
