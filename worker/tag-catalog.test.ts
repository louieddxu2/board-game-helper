...
describe('versioned public tag catalog', () => {
  test('reads only catalog versions newer than the client cursor without joining source tables', async () => {
    const all = vi.fn().mockResolvedValue({ results: [{
      ...
      entry_json: JSON.stringify({ id: 'tag_draw', slug: 'draw', name: '抽牌', aliases: ['補牌'], categoryHints: ['action_effect_detail'], detectionKeywords: ['抽一張牌'], isPublic: true, updatedAt: 10 }),
      deleted: 0,
    }], meta: { rows_read: 1 } });
    ...
    expect(payload).toEqual({
      changes: [{ tagId: 'tag_draw', catalogVersion: 8, deleted: false, tag: expect.objectContaining({ name: '抽牌', aliases: ['補牌'], categoryHints: ['action_effect_detail'], detectionKeywords: ['抽一張牌'] }) }],
      throughVersion: 8,
      hasMore: false,
    });
  });
