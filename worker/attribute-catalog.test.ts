...
describe('versioned attribute table catalog', () => {
  ...
  test('decodes value, subject, candidate, and deletion deltas', () => {
    const payload = attributeCatalogChangesPayload({ results: [
      ...
      { entry_key: 'candidate:candidate-1', catalog_version: 15, entry_json: JSON.stringify({ kind: 'candidate', id: 'candidate-1', displayName: '待處理', valuesJson: JSON.stringify([8]), matchStatus: 'pending', sourceRowNumber: 3 }), deleted: 0 },
      { entry_key: 'value:subject-b:attribute-luck', catalog_version: 16, entry_json: null, deleted: 1 },
    ] }, 12);

    expect(payload.throughVersion).toBe(16);
    ...
    expect(payload.changes[2]).toMatchObject({ candidate: expect.objectContaining({ id: 'candidate-1', values: [8] }) });
    expect(payload.changes[3]).toMatchObject({ deleted: true });
  });
